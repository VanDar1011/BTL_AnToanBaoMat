...
//                buffer[bufferIndex++] = byte1;
               ...
               // buffer[bufferIndex++] = byte2;
                char hexPair[3]; // Mảng để lưu một cặp ký tự hexa và kết thúc chuỗi

    // Đọc các cặp ký tự hexadecimals từ tệp và lưu vào mảng buffer
    while (fscanf(inputFileCir, "%2s", hexPair) == 1) {
        unsigned char byte;
        sscanf(hexPair, "%hhx", &byte);
